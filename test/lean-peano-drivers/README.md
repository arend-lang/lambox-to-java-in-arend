# Drivers for axiom-free (Peano) Lean cases — the only way the C backend sees a Lean program
...
module name `bench` so any case can share them):
...
| `empty.attr` | `(attributes_config () () () () ())` — no remapping; note `peregrine` rejects a zero-byte file |
| `peano-case.sh` | the shared `case.sh` fragment |
...
(`cases/lean-rbmap-mono`) covers the program on java/ocaml.

**Beware when writing a new case:** a large numeric *literal* in the Lean source
